...
async function startServer() {
    try {
        ...
        app.post('/api/add-set-to-user/:setKey', async (req, res) => { /* 이전과 동일 */ });
        app.post('/api/correct/update', async (req, res) => { /* 이전과 동일 */ });
        ...
        // 새로운 API: 전체 단어 DB에서 무작위로 단어를 추출하여 학습 목록을 덮어쓰기
        app.post('/api/userdata/random-set', async (req, res) => {
            try {
                ...
                // 3. 배열을 무작위로 섞기 (Fisher-Yates shuffle)
                ...
                // 5. 사용자의 학습 목록을 이 랜덤 샘플로 덮어쓰기
                await userdata.updateOne(
                    { _id: 'main' },
                    { $set: { 'data.vocabularyData': randomSample, 'data.addedSets': [] } }, // addedSets는 초기화
                    { upsert: true }
                );
...
// (편의상 생략된 함수들의 전체 코드)
app.get('/api/userdata', async (req, res) => { try { let result = await userdata.findOne({ _id: 'main' }); if (!result || !result.data) { const initialData = { vocabularyData: [], addedSets: [], incorrectCounts: {}, correctCounts: {} }; await userdata.updateOne({ _id: 'main' }, { $set: { data: initialData } }, { upsert: true }); result = { data: initialData }; } if (!result.data.correctCounts) result.data.correctCounts = {}; res.json(result.data); } catch (e) { res.status(500).json({ message: "사용자 데이터 조회 오류" }); } });
...
app.post('/api/wordsets', async (req, res) => { try { const { key, content } = req.body; await wordsets.updateOne({ _id: key }, { $set: { content } }, { upsert: true }); res.status(201).json({ message: `${key}번 세트 저장 성공` }); } catch (e) { res.status(500).json({ message: "단어 세트 저장 오류" }); } });
app.post('/api/add-set-to-user/:setKey', async (req, res) => { const { setKey } = req.params; try { const wordSet = await wordsets.findOne({ _id: setKey }); if (!wordSet) return res.status(404).json({ message: '세트를 찾을 수 없습니다.' }); const lines = wordSet.content.split('\n').filter(line => line.trim()); const wordsFromSet = []; const japaneseRegex = /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/; lines.forEach(line => { const parts = line.split(',').map(part => part.trim()); if (parts.length >= 4) { let japanese, korean, hiragana, pronunciation, kanjiReadings; if (japaneseRegex.test(parts[0])) { [japanese, korean, hiragana, pronunciation, ...kanjiReadings] = parts; } else { [korean, japanese, hiragana, pronunciation, ...kanjiReadings] = parts; } const finalParts = [korean, hiragana, pronunciation, ...kanjiReadings]; wordsFromSet.push({ id: crypto.randomUUID(), japanese: japanese, parts: finalParts }); } }); const userDoc = await userdata.findOne({ _id: 'main' }); const currentVocab = userDoc.data.vocabularyData || []; const uniqueNewWords = wordsFromSet.filter(nw => !currentVocab.some(ew => ew.japanese === nw.japanese)); const updateQuery = { $addToSet: { 'data.addedSets': setKey } }; if (uniqueNewWords.length > 0) { updateQuery.$push = { 'data.vocabularyData': { $each: uniqueNewWords } }; } await userdata.updateOne({ _id: 'main' }, updateQuery, { upsert: true }); res.status(200).json({ message: '학습 목록 추가 성공' }); } catch (e) { console.error(e); res.status(500).json({ message: "학습 목록 추가 오류" }); } });
app.post('/api/correct/update', async (req, res) => { const { word, count } = req.body; try { await userdata.updateOne({ _id: 'main' }, { $set: { [`data.correctCounts.${word}`]: count } }, { upsert: true }); res.status(200).json({ message: '정답 횟수 업데이트 성공' }); } catch (e) { res.status(500).json({ message: "정답 횟수 업데이트 중 오류" }); } });
app.post('/api/incorrect/update', async (req, res) => { const { word, count } = req.body; try { await userdata.updateOne({ _id: 'main' }, { $set: { [`data.incorrectCounts.${word}`]: count } }, { upsert: true }); res.status(200).json({ message: '오답 횟수 업데이트 성공' }); } catch (e) { res.status(500).json({ message: "오답 횟수 업데이트 중 오류" }); } });
