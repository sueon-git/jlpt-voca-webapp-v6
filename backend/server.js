    ...
    try {
        ...
        app.get('/api/userdata', async (req, res) => {
            try {
                ...
                if (!result || !result.data) {
                    // correctCounts 필드가 없는 초기 데이터 구조
                    const initialData = { 
                        vocabularyData: [], 
                        addedSets: [], 
                        incorrectCounts: {},
                        correctCounts: {} // 정답 카운트 추가
                    };
                ...
                // 이미 데이터가 있을 때 correctCounts가 없는 경우 대비
                if (!result.data.correctCounts) {
                    result.data.correctCounts = {};
                }
        ...
        app.post('/api/correct/update', async (req, res) => {
            ...
            try {
                await userdata.updateOne(
                    { _id: 'main' },
                    { $set: { [`data.correctCounts.${word}`]: count } }
                );
